...
def LP(g, log_flag=0, var_details=0):
    ...
    max_density = floor(model_1.objVal)
    ...
    if len(g.densest_set) == 0:
        print("Error! len(g.densest_set) == 0")
        exit()
    g_ = g.subgraph(g.densest_set)
    if not approximate_equal(len(g_.edge_list) / len(g_.node_list), max_density):
        print("Error! Obj != density")
        exit()
...
def find_all_minimal(PDB_id, t1, t2, step):
    # if PDB_id != "2B10_o":
    #     return
    tolerance = t2
    ...
    while True:
        ...
        if temp_result == None:
            while tolerance >= t1:
                if not os.path.exists("theta/{}".format(tolerance)):
                    os.mkdir("theta/{}".format(tolerance))
                with open("theta/{}/{}_minimal_sub.txt".format(tolerance, PDB_id), "w") as f1:
                    for i in result:
                        f1.write("{}\n".format(i.id))
                tolerance -= step
        ...
        if temp_result.max_density < max_density * tolerance:
            while temp_result.max_density < max_density * tolerance:
                if not os.path.exists("theta/{}".format(tolerance)):
                    os.mkdir("theta/{}".format(tolerance))
                with open("theta/{}/{}_minimal_sub.txt".format(tolerance, PDB_id), "w") as f1:
                    for i in result:
        ...
        if remained_nodes == None:
            ...
            break
        temp_graph = g_.subgraph(remained_nodes)
        temp_graph.update()
        LP(temp_graph)
        g_ = copy.deepcopy(temp_graph)
...
def theta_test(t1, t2, step=0.01):
    ...
    for path, dir_list, file_list in graph_files:
        ...
        for file_name in file_list:
            ...
            find_all_minimal(temp_pdb_id, t1, t2, step)
            count += 1

theta_test(0.5, 1.00, 0.01)

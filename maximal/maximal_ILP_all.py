# This file is different to maximal_ILP_all.py
# In ILP part, y_i - z_i < epsilon restriction is removed
# Results saved to result3/
...
def ILP(g, max_density):
    ...
    for i in n_list:
        variables_y_dict["{}".format(i.id)] = model_1.addVar(vtype=GRB.CONTINUOUS, lb=0, ub=1, name="y-{}".format(i.id))
        variables_z_dict["{}".format(i.id)] = model_1.addVar(vtype=GRB.BINARY, name="z-{}".format(i.id))

    variables_x_list = list(variables_x_dict[i] for i in variables_x_dict)
    variables_y_list = list(variables_y_dict[i] for i in variables_y_dict)
    variables_z_list = list(variables_z_dict[i] for i in variables_z_dict)
    ...
    for i in n_list:
        model_1.addConstr(
            variables_y_dict["{}".format(i.id)] - variables_z_dict["{}".format(i.id)] / len(n_list) >= 0,
            "c-{}_1".format(i.id))
        # model_1.addConstr(
        #     variables_y_dict["{}".format(i.id)] - variables_z_dict["{}".format(i.id)] <= epsilon,
        #     "c-{}_1".format(i.id))

    model_1.addConstr(quicksum(variables_x_list) >= max_density)

    model_1.setObjective(quicksum(variables_z_list), GRB.MAXIMIZE)
...
def find_maximal(PDB_id):
    ...
    g_ = preprocess(g)
    result = ILP(g_, g_.max_density)

    with open("result3/{}_maximal.txt".format(PDB_id), "w") as f1:
        for i in result:
            f1.write("{}\n".format(i))
